<template>
  <div class="daice-chart-card mb-4">
    <div class="daice-chart-header">
      ...
      <div>
        <h3 class="daice-chart-title">Volume by Dealer</h3>
        <p class="daice-chart-subtitle">
          Dealers ranked by order volume from Da ICE Customer app users from
...
const { fetchVolumeByDealer } = useDashboardCards();
    ...
    tooltip: {
      ...
      callbacks: {
        label(context) {
          ...
          return ` ${count.toLocaleString()} ${count <= 1 ? "Kg" : "Kgs"}`;
        },
      },
    },
  },
...
async function loadChart(startDate, endDate) {
  const records = await fetchVolumeByDealer({
